<template>
  <div class="new-cuota-medidor-page">
    <div class="page-header">
      <h1 class="page-title">NUEVO REGISTRO DE CUOTAS MEDIDOR</h1>
...
const handleSubmit = async () => {
  try {
    ...
    await cuotasMedidorService.create(cuotaData);
    
    $q.notify({
      type: 'positive',
      message: 'Cuota medidor creada exitosamente',
      position: 'center'
    });
    
    // Redirigir a la página de cuotas medidor
    router.push('/diferidos/cuotas-medidor');
  } catch (error) {
    console.error('Error al crear cuota medidor:', error);
    $q.notify({
      type: 'negative',
      message: 'Error al crear la cuota medidor',
