<template>
  <div class="new-cuota-conexion-page">
    <div class="page-header">
      <h1 class="page-title">NUEVO REGISTRO DE CUOTAS CONEXION</h1>
...
const handleSubmit = async () => {
  try {
    ...
    await cuotasConexionService.create(cuotaData);
    
    $q.notify({
      type: 'positive',
      message: 'Cuota conexión creada exitosamente',
      position: 'center'
    });
    
    // Redirigir a la página de cuotas conexión
    router.push('/diferidos/cuotas-conexion');
  } catch (error) {
    console.error('Error al crear cuota conexión:', error);
    $q.notify({
      type: 'negative',
      message: 'Error al crear la cuota conexión',
